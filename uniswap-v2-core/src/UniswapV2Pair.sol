...
contract UniswapV2Pair is IUniswapV2Pair, UniswapV2ERC20 {
    ...
    uint public constant MINIMUM_LIQUIDITY = 10 ** 3; // 최소 유동성 공급량 (1,000)
    ...
    uint32 private blockTimestampLast; // 지난 블록에서의 업데이트 시간

    uint public price0CumulativeLast; // 토큰0의 누적 가격
    uint public price1CumulativeLast; // 토큰1의 누적 가격
    uint public kLast; // 가장 최근의 유동성 변경 이후의 상수 k (reserve0 * reserve1)
    ...
    function _update(
        ...
        uint32 timeElapsed = blockTimestamp - blockTimestampLast; // 지난 블록에서의 업데이트 시간과 현재 블록에서의 업데이트 시간의 차이 계산
        if (timeElapsed > 0 && _reserve0 != 0 && _reserve1 != 0) {
            // 지난 호출 시의 블록 번호와 현재 블록 번호가 다르고, 토큰0과 토큰1의 보유량이 0이 아니면 가격 누적값을 업데이트 (현 블록에서 처음 호출된 경우)
            // timeElapsed = 새로운 블록에서의 첫 번째 업데이트 시간 - 지난 블록에서의 마지막 업데이트 시간
    ...
    function _mintFee(
        ...
        if (feeOn) {
            // 수수료 수취인 주소가 0이 아니면
            if (_kLast != 0) {
                // 가장 최근의 유동성 변경 이후의 상수 k가 0이 아니면
                ...
                if (rootK > rootKLast) {
                    ...
                    if (liquidity > 0) _mint(feeTo, liquidity); // 수수료 수취인 주소로 liquidity만큼의 UniswapV2ERC20 토큰을 전송
                }
            }
        } else if (_kLast != 0) {
            // 수수료 수취인 주소가 0이고, 가장 최근의 유동성 변경 이후의 상수 k가 0이 아니면
            kLast = 0; // 가장 최근의 유동성 변경 이후의 상수 k를 0으로 업데이트
        }
    }

    // this low-level function should be called from a contract which performs important safety checks
    function mint(address to) external lock returns (uint liquidity) {
        (uint112 _reserve0, uint112 _reserve1, ) = getReserves(); // gas savings (추적된 토큰0과 토큰1의 보유량)
        uint balance0 = IERC20(token0).balanceOf(address(this)); // 현재 토큰0의 보유량 (추적되지 않은 보유량이 포함될 수 있음)
        uint balance1 = IERC20(token1).balanceOf(address(this)); // 현재 토큰1의 보유량 (추적되지 않은 보유량이 포함될 수 있음)
        uint amount0 = balance0.sub(_reserve0); // 추적되지 않은 토큰0의 보유량
        uint amount1 = balance1.sub(_reserve1); // 추적되지 않은 토큰1의 보유량
        ...
        require(liquidity > 0, "UniswapV2: INSUFFICIENT_LIQUIDITY_MINTED"); // 유동성 공급량이 0이면 에러
        _mint(to, liquidity); // to로 liquidity만큼의 UniswapV2ERC20 토큰을 전송
    ...
    function burn(
        ...
        uint liquidity = balanceOf[address(this)]; // 현재 컨트랙트의 유동성 공급량
        ...
        ); // 토큰0이나 토큰1의 보유량이 0이면 에러
        _burn(address(this), liquidity); // 현재 컨트랙트의 유동성 공급량만큼의 UniswapV2ERC20 토큰을 소각
    ...
    // this low-level function should be called from a contract which performs important safety checks
    function swap(
        uint amount0Out, // 풀에서 빠져나갈 토큰0의 수량 (토큰1)
        uint amount1Out, // 풀에서 빠져나갈 토큰1의 수량
        address to, // 전송 수취인 주소
        bytes calldata data
    ) external lock {
        require(
    ...
    // 토큰0과 토큰1의 보유량이 추적된 토큰0과 토큰1의 보유량과 일치하도록 남은 토큰을 to로 전송
    ...
    // 추적된 토큰0과 토큰1의 보유량을 현재 토큰0과 토큰1의 보유량으로 업데이트 (추적되지 않은 보유량이 포함될 수 있음)
