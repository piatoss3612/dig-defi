...
interface IUniswapV2Pair {
    ...
    function totalSupply() external view returns (uint256); // 토큰의 총 공급량 (getter)
    ...
    function MINIMUM_LIQUIDITY() external pure returns (uint256); // 최소 유동성
    ...
    function getReserves() external view returns (uint112 reserve0, uint112 reserve1, uint32 blockTimestampLast); // 토큰0과 토큰1의 잔액과 마지막 업데이트 시간 (getter)

    function price0CumulativeLast() external view returns (uint256); // 토큰0의 누적 가격 (getter)

    function price1CumulativeLast() external view returns (uint256); // 토큰1의 누적 가격 (getter)

    function kLast() external view returns (uint256); // 상수 k의 마지막 값 (getter)

    function mint(address to) external returns (uint256 liquidity); // to에게 토큰을 발행

    function burn(address to) external returns (uint256 amount0, uint256 amount1); // to에게 토큰을 인출

    function swap(uint256 amount0Out, uint256 amount1Out, address to, bytes calldata data) external; // to에게 amount0Out만큼의 token0과 amount1Out만큼의 token1을 일정한 비율로 스왑 (data는 IUniswapV2Callee.uniswapV2Call()을 호출하기 위한 데이터)
