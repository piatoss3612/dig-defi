...
contract UniswapV2Pair is UniswapV2ERC20 {
    ...
    uint256 public constant MINIMUM_LIQUIDITY = 10 ** 3; // 최소 유동성 (1,000)
    bytes4 private constant SELECTOR = bytes4(keccak256(bytes("transfer(address,uint256)"))); // transfer 함수의 selector (transfer.selector를 사용할 수도 있음)
    ...
    uint256 public price0CumulativeLast; // 토큰0의 누적 가격
    uint256 public price1CumulativeLast; // 토큰1의 누적 가격
    uint256 public kLast; // 마지막으로 유동성 풀이 업데이트된 이후의 상수 k

    // IUniswapV2Pair 인터페이스의 이벤트
    event Mint(address indexed sender, uint256 amount0, uint256 amount1); // Mint 이벤트 (sender가 amount0만큼의 token0과 amount1만큼의 token1을 유동성 풀에 추가)
    event Burn(address indexed sender, uint256 amount0, uint256 amount1, address indexed to); // Burn 이벤트 (sender가 amount0만큼의 token0과 amount1만큼의 token1을 유동성 풀에서 인출)
    event Swap(
    ...
    function _update(
        uint256 balance0, // 새로운 토큰0의 보유량
        uint256 balance1, // 새로운 토큰1의 보유량
        uint112 _reserve0, // 기존의 토큰0의 보유량
        uint112 _reserve1 // 기존의 토큰1의 보유량
    ) private {
        require(balance0 <= type(uint112).max && balance1 <= type(uint112).max, "UniswapV2: OVERFLOW"); // uint112 타입의 최댓값보다 보유량이 크면 에러
        ...
        if (timeElapsed > 0 && _reserve0 != 0 && _reserve1 != 0) {
            ...
            price0CumulativeLast += uint256(UQ112x112.encode(_reserve1).uqdiv(_reserve0)) * timeElapsed; // price0 = reserve1 / reserve0, price0CumulativeLast += price0 * timeElapsed
            price1CumulativeLast += uint256(UQ112x112.encode(_reserve0).uqdiv(_reserve1)) * timeElapsed; // price1 = reserve0 / reserve1, price1CumulativeLast += price1 * timeElapsed
        }
    ...
    function _mintFee(
        ...
        uint256 _kLast = kLast; // kLast를 불러옴
        if (feeOn) {
            // 수수료 수취인 주소가 설정되어 있는 경우에만 수행
            if (_kLast != 0) {
                // 지난번 유동성 변경 이후의 상수 k가 0이 아니면
                uint256 rootK = Math.sqrt(uint256(_reserve0).mul(_reserve1)); // rootK = sqrt(reserve0 * reserve1)
                uint256 rootKLast = Math.sqrt(_kLast); // rootKLast = sqrt(kLast)

                if (rootK > rootKLast) {
                    uint256 numerator = totalSupply.mul(rootK.sub(rootKLast)); // numerator = totalSupply * (sqrt(reserve0 * reserve1) - sqrt(kLast))
                    uint256 denominator = rootK.mul(5).add(rootKLast); // denominator = 5 * sqrt(reserve0 * reserve1) + sqrt(kLast)
                    uint256 liquidity = numerator / denominator; // liquidity = numerator / denominator = totalSupply * (sqrt(reserve0 * reserve1) - sqrt(kLast)) / (5 * sqrt(reserve0 * reserve1) + sqrt(kLast))
    ...
    function mint(address to) external lock returns (uint256 liquidity) {
        ...
        uint256 balance0 = IERC20(token0).balanceOf(address(this)); // 현재 토큰0의 보유량 (새롭게 유동성 풀에 공급된 토큰0의 보유량이 포함될 수 있음)
        uint256 balance1 = IERC20(token1).balanceOf(address(this)); // 현재 토큰1의 보유량 (새롭게 유동성 풀에 공급된 토큰1의 보유량이 포함될 수 있음)
        uint256 amount0 = balance0.sub(_reserve0); // 새롭게 유동성 풀에 공급된 토큰0의 보유량
        uint256 amount1 = balance1.sub(_reserve1); // 새롭게 유동성 풀에 공급된 토큰1의 보유량

        bool feeOn = _mintFee(_reserve0, _reserve1); // 수수료가 적용되는지 확인
        uint256 _totalSupply = totalSupply; // _mintFee 함수에서 totalSupply를 업데이트할 수 있으므로, 호출하고 난 후에 불러와야 함
        if (_totalSupply == 0) {
            ...
            liquidity = Math.min(amount0.mul(_totalSupply) / _reserve0, amount1.mul(_totalSupply) / _reserve1); // liquidity = min(amount0 * totalSupply / reserve0, amount1 * totalSupply / reserve1)
        }
        ...
        if (feeOn) kLast = uint256(reserve0).mul(reserve1); // 수수료가 적용되면, 최근 상수 k를 업데이트
    ...
    function burn(address to) external lock returns (uint256 amount0, uint256 amount1) {
        ...
        uint256 balance0 = IERC20(_token0).balanceOf(address(this)); // 현재 토큰0의 보유량 (추적되지 않은 보유량이 포함될 수 있음)
        uint256 balance1 = IERC20(_token1).balanceOf(address(this)); // 현재 토큰1의 보유량 (추적되지 않은 보유량이 포함될 수 있음)
        uint256 liquidity = balanceOf[address(this)]; // 현재 컨트랙트의 LP 토큰 보유량 (= to가 반환한 LP 토큰의 수량)

        bool feeOn = _mintFee(_reserve0, _reserve1); // 수수료가 적용되는지 확인
        uint256 _totalSupply = totalSupply; // _mintFee 함수에서 수수료가 발행되면 totalSupply가 업데이트될 수 있으므로, 호출하고 난 후에 불러와야 함
        ...
        require(amount0 > 0 && amount1 > 0, "UniswapV2: INSUFFICIENT_LIQUIDITY_BURNED"); // 어느 하나라도 0이면 에러
        ...
        _update(balance0, balance1, _reserve0, _reserve1); // 토큰0과 토큰1의 보유량을 업데이트
        if (feeOn) kLast = uint256(reserve0).mul(reserve1); // reserve0와 reserve1이 업데이트되었으므로, 최근 상수 k를 업데이트
        emit Burn(msg.sender, amount0, amount1, to); // Burn 이벤트 발생
    }

    // this low-level function should be called from a contract which performs important safety checks
    function swap(
        uint256 amount0Out, // 풀에서 빠져나갈 토큰0의 수량
        uint256 amount1Out, // 풀에서 빠져나갈 토큰1의 수량
        address to, // 토큰을 전송할 주소
        bytes calldata data // IUniswapV2Callee.uniswapV2Call()을 호출하기 위한 데이터
    ) external lock {
        require(amount0Out > 0 || amount1Out > 0, "UniswapV2: INSUFFICIENT_OUTPUT_AMOUNT"); // 토큰0과 토큰1의 수량이 모두 0이면 에러
        (uint112 _reserve0, uint112 _reserve1,) = getReserves(); // 추적된 토큰0과 토큰1의 보유량
        require(amount0Out < _reserve0 && amount1Out < _reserve1, "UniswapV2: INSUFFICIENT_LIQUIDITY"); // 토큰0과 토큰1의 보유량보다 많은 수량을 빼려고 하면 에러
        ...
        uint256 amount0In = balance0 > _reserve0 - amount0Out ? balance0 - (_reserve0 - amount0Out) : 0; // swap을 통해 유동성 풀에 추가된 토큰0의 수량
        uint256 amount1In = balance1 > _reserve1 - amount1Out ? balance1 - (_reserve1 - amount1Out) : 0; // swap을 통해 유동성 풀에 추가된 토큰1의 수량
        require(amount0In > 0 || amount1In > 0, "UniswapV2: INSUFFICIENT_INPUT_AMOUNT"); // swap을 통해 유동성 풀에 추가된 토큰0과 토큰1의 수량이 하나라도 0이면 에러 반환
        {
            // scope for reserve{0,1}Adjusted, avoids stack too deep errors
            uint256 balance0Adjusted = balance0.mul(1000).sub(amount0In.mul(3)); // amount0In이 0이 아니라면 0.3%의 수수료가 적용되지 않은 토큰0의 보유량
            uint256 balance1Adjusted = balance1.mul(1000).sub(amount1In.mul(3)); // amount1In이 0이 아니라면 0.3%의 수수료가 적용되지 않은 토큰1의 보유량
            require(
                balance0Adjusted.mul(balance1Adjusted) >= uint256(_reserve0).mul(_reserve1).mul(1000 ** 2), // 수수료가 적용되지 않은 토큰0과 토큰1의 보유량의 곱이 기존의 토큰0과 토큰1의 보유량의 곱보다 크거나 같아야 함 (수수료 포함 여부 확인)
                "UniswapV2: K"
            ); // 상수 k가 변하지 않도록 체크
        }
