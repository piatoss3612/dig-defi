...
contract UniswapV2Router01 is IUniswapV2Router01 {
    ...
    function removeLiquidity(
        ...
        (uint amount0, uint amount1) = IUniswapV2Pair(pair).burn(to); // 토큰 A와 토큰 B를 받음
    ...
    function removeLiquidityWithPermit(
        ...
        address pair = UniswapV2Library.pairFor(factory, tokenA, tokenB);
        uint value = approveMax ? type(uint).max : liquidity; // approveMax가 true면 uint의 최대값, 아니면 liquidity
        IUniswapV2Pair(pair).permit(
            ...
            r,
            s
        ); // permit 함수 호출해 라우터가 대신 유동성 토큰을 전송할 수 있도록 허용
        ...
        returns (uint[] memory amounts)
    {
        ...
        require(
            amounts[0] <= msg.value,
            "UniswapV2Router: EXCESSIVE_INPUT_AMOUNT"
        ); // 교환할 토큰의 양이 msg.value보다 작거나 같은지 확인
        IWETH(WETH).deposit{value: amounts[0]}(); // ETH를 WETH로 전환
        assert(
            IWETH(WETH).transfer(
                UniswapV2Library.pairFor(factory, path[0], path[1]),
